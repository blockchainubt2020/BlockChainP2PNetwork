...
class Blockchain:
    ...
    def create_block(self, nonce, previous_hash):
        """
        Add a block transactions to the blockchain
        """
        block = {
    ...
    def submit_transaction(self, sender_public_key, recipient_public_key, signature, amount):
        ...
        # Rewar for mining block
        if sender_public_key == MINING_SENDER:
            self.transactions.append(transaction)
            return len(self.chain) + 1
        else:
            # Transacttion from wallet to another Wallet
...
def mine():
    ...
    blockchain.submit_transaction(
        sender_public_key=MINING_SENDER, recipient_public_key=blockchain.node_id,
        signature='',
...
def new_transaction():
    ...
    transaction_results = blockchain.submit_transaction(
        values['confirmation_sender_public_key'], values['confirmation_recipient_public_key'], values['transaction_signature'], values['confirmation_amount'])
